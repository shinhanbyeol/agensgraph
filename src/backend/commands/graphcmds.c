				   ...
				   int stmt_location, int stmt_len, ParamListInfo params)
{
	...
	foreach(l, stmts)
	{
		...
		if (IsA(stmt, CreateStmt))
		{
			...
			ObjectAddress reladdr = DefineRelation(stmt, RELKIND_RELATION,
												   InvalidOid, NULL,
												   queryString);
			Datum		toast_options;
			...
			DefineLabel((CreateStmt *) stmt, &reladdr, labkind,
...
ObjectAddress
DefineLabel(CreateStmt *stmt, ObjectAddress *reladdr, char labkind,
