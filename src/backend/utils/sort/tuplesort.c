...
Tuplesortstate *
tuplesort_begin_custom(TupleDesc tupDesc, bool enforceUnique,
					   ...
					   void *arg)
{
	...
	if (trace_sort)
		elog(LOG,
			 "begin custom sort: nkeys = %d, workMem = %d, randomAccess = %c",
			 nkeys, workMem, randomAccess ? 't' : 'f');
#endif
	...
	state->comparetup = comparetup;
	state->copytup = copytup;
	state->writetup = writetup;
	state->readtup = readtup;
	state->arg = arg;
